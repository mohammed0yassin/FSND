...
AUTH0_DOMAIN = 'ud-coffee.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'coffee'
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header must contain the permissions.'
        }, 401)

    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'missing_permission',
            'description': 'the current user has insufficient permissions to perform the requested operation.'
        }, 401)
